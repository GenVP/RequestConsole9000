...
// (C) Александр Кузнецов 2019-2021
// hal@hal9000.cc
...
Процедура Инициализация(Форма = Неопределено, чСеансИД = Неопределено) Экспорт
	...
	Попытка
		...
		Если Сред(ИспользуемоеИмяФайлаВнешнейОбработки, 2, 2) = ":" + ПолучитьРазделительПути() ИЛИ Лев(ИспользуемоеИмяФайлаВнешнейОбработки, 1) = ПолучитьРазделительПути() Тогда
...
Функция ДанныеВременныхТаблиц(МенеджерВременныхТаблиц, ТолькоДляТекстаЗапроса = Неопределено) Экспорт
	...
	ИспользуемыеТаблицы = Новый Соответствие;
	Если ТолькоДляТекстаЗапроса <> Неопределено Тогда
		СоздаваемыеТаблицы = Новый Соответствие;
		
		СхемаЗапроса = Новый СхемаЗапроса;
		СхемаЗапроса.УстановитьТекстЗапроса(ТолькоДляТекстаЗапроса);
		
		Для Каждого ОписаниеПакетЗапросов Из СхемаЗапроса.ПакетЗапросов Цикл
			Если ТипЗнч(ОписаниеПакетЗапросов) = Тип("ЗапросУничтоженияТаблицыСхемыЗапроса") Тогда
				Если СоздаваемыеТаблицы[ОписаниеПакетЗапросов.ИмяТаблицы] = Истина Тогда
					СоздаваемыеТаблицы.Удалить(ОписаниеПакетЗапросов.ИмяТаблицы);
				КонецЕсли;
			Иначе
				Для Каждого ОписаниеОператор Из ОписаниеПакетЗапросов.Операторы Цикл
					Для Каждого ОписаниеИстоочника Из ОписаниеОператор.Источники Цикл
						Если ТипЗнч(ОписаниеИстоочника.Источник) = Тип("ОписаниеВременнойТаблицыСхемыЗапроса") Тогда
							ИмяТаблицы = ОписаниеИстоочника.Источник.ИмяТаблицы;
							Если СоздаваемыеТаблицы[ИмяТаблицы] = Неопределено
							   И ИспользуемыеТаблицы[ИмяТаблицы] = Неопределено Тогда
								ИспользуемыеТаблицы.Вставить(ИмяТаблицы, Истина);
							КонецЕсли;
						КонецЕсли;
					КонецЦикла;
				КонецЦикла;
				Если НЕ ПустаяСтрока(ОписаниеПакетЗапросов.ТаблицаДляПомещения)
				   И СоздаваемыеТаблицы[ОписаниеПакетЗапросов.ТаблицаДляПомещения] = Неопределено Тогда
					СоздаваемыеТаблицы.Вставить(ОписаниеПакетЗапросов.ТаблицаДляПомещения, Истина);
				КонецЕсли;
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
	...
	Для Каждого Таблица Из МенеджерВременныхТаблиц.Таблицы Цикл
		...
		Если ТолькоДляТекстаЗапроса <> Неопределено И ИспользуемыеТаблицы[Таблица.ПолноеИмя] = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		...
		ВременныеТаблицы.Добавить(Новый Структура("Имя, Таблица", Таблица.ПолноеИмя, ВременнаяТаблица));
	КонецЦикла;
...
Функция СохранитьЗапрос(СеансИД, Запрос, ВсеВременныеТаблицы = Истина) Экспорт
	...
	ВременныеТаблицы = ДанныеВременныхТаблиц(Запрос.МенеджерВременныхТаблиц, ?(ВсеВременныеТаблицы, Неопределено, Запрос.Текст));
	...
	Если ВременныеТаблицы <> Неопределено Тогда
		Структура.Вставить("ВременныеТаблицы", ВременныеТаблицы);
	КонецЕсли;
...
Функция СохранитьЗапросСКД(СеансИД, СхемаКомпоновкиДанных, КомпоновщикНастроек = Неопределено, ДанныеРасшифровки = Неопределено) Экспорт
	...
	Если ТипЗнч(СхемаКомпоновкиДанных) = Тип("МакетКомпоновкиДанных") Тогда
	...
	Для Каждого ЭлементПараметра Из МакетКомпоновки.ЗначенияПараметров Цикл
		ПараметрыЗапросов.Вставить(ЭлементПараметра.Имя, ЭлементПараметра.Значение);
	КонецЦикла;
	
	ПрефиксиИмени = "";
	Для Каждого ЭлементНабора Из МакетКомпоновки.НаборыДанных Цикл
		ВыгрузитьЗапросНабораДанныхСКД(ЭлементНабора, ПараметрыЗапросов, ПрефиксиИмени);
	КонецЦикла;
...
Процедура ВыгрузитьЗапросНабораДанныхСКД(НаборДанных, ПараметрыЗапросов, ПрефиксИмени)
	
	Если ТипЗнч(НаборДанных) = Тип("НаборДанныхОбъединениеМакетаКомпоновкиДанных") Тогда
		ПрефиксиИмениНабора = ПрефиксИмени + НаборДанных.Имя + ".";
		Для Каждого ЭлементНабора Из НаборДанных.Элементы Цикл
			ВыгрузитьЗапросНабораДанныхСКД(ЭлементНабора, ПараметрыЗапросов, ПрефиксиИмениНабора);
		КонецЦикла;
	ИначеЕсли ТипЗнч(НаборДанных) = Тип("НаборДанныхЗапросМакетаКомпоновкиДанных") Тогда
		ОписаниеЗапроса = Новый Структура("Текст,Параметры", НаборДанных.Запрос, Новый Структура);
		Для Каждого КлючИЗначение Из ПараметрыЗапросов Цикл
			ОписаниеЗапроса.Параметры.Вставить(КлючИЗначение.Ключ, КлючИЗначение.Значение);
		КонецЦикла;
		Для Каждого ЭлементПараметра Из НаборДанных.ЗначенияПараметров Цикл
			ОписаниеЗапроса.параметры.Вставить(ЭлементПараметра.Имя, ЭлементПараметра.Значение);
		КонецЦикла;
		
		ИмяФайла = ПолучитьИмяВременногоФайла(РасширениеЗахваченныхЗапросов);
		ЗаписьXML = Новый ЗаписьXML;
		ЗаписьXML.ОткрытьФайл(ИмяФайла);
		СериализаторXDTO.ЗаписатьXML(ЗаписьXML, ОписаниеЗапроса, НазначениеТипаXML.Явное);
		ЗаписьXML.Закрыть();
	КонецЕсли;
		...
		Для Каждого Строка Из тзСтруктура Цикл
			...
			Если ъ > 0 Тогда
				...
				Если НЕ ЗначениеЗаполнено(ИмяТаблицы) Тогда
					ИмяТаблицы = СтрШаблон("%1_%2", Строка.Метаданные, Строка.Назначение);
				КонецЕсли;
				
				SQLЗапросВТермины1С_ДобавитьТермин(ДанныеТерминов, Строка.ИмяТаблицыХранения, ИмяТаблицы);
