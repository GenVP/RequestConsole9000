...
&НаКлиенте
Процедура ПереключитьВидимостьСпискаЗапросов(Команда)
	Элементы.ВидимостьСпискаЗапросов.Пометка = НЕ Элементы.ВидимостьСпискаЗапросов.Пометка;
	Элементы.ГруппаСписокЗапросов.Вимдимость = Элементы.ВидимостьСпискаЗапросов.Пометка;
КонецПроцедуры

&НаКлиенте
Процедура ПереключитьВидимостьНастроекЗапроса(Команда)
	Элементы.ВидимостьНастроекЗапроса.Пометка = НЕ Элементы.ВидимостьНастроекЗапроса.Пометка;
	Элементы.ВидимостьСпискаЗапросов.Пометка = НЕ Элементы.ВидимостьНастроекЗапроса.Пометка;
	Элементы.ГруппаСписокЗапросов.Вимдимость = Элементы.ВидимостьСпискаЗапросов.Пометка;
	Элементы.ГруппаВерхняяПанель.Видимость = Элементы.ВидимостьНастроекЗапроса.Пометка;
КонецПроцедуры

&НаКлиенте
Процедура ВыполнитьАлгоритмЗаполнения(Команда)
...
Процедура ВыполнитьАлгоритмЗаполненияНаСервере()
	...
	БезопасноВыполнитьАлгоритмЗаполненияНаСервере(ТекстКодЗаполнения, ТекстЗапросаСтрокой, СтруктураПараметров);
	ТекстЗапроса.УстановитьТекст(ТекстЗапросаСтрокой);
	ОбновитьПараметрыЗапросаПоСтруктуре(ЭтотОбъект, ТекстЗапросаСтрокой, СтруктураПараметров);
КонецПроцедуры

&НаСервереБезКонтекста
Процедура БезопасноВыполнитьАлгоритмЗаполненияНаСервере(ЭтотКод, ТекстЗапроса, Параметры)
	Выполнить(ЭтотКод);
КонецПроцедуры

&НаСервере
Процедура ОбновитьПараметрыЗапросаПоСтруктуре(Форма, ТекстЗапросаСтрокой, СтруктураПараметров)
	
	ОбработкаОбъект = Неопределено;
	
	Форма.ТекстЗапроса = ТекстЗапроса;
	
	Для Каждого СтрокаПараметра Из Форма.ПараметрыЗапроса Цикл
		
		ЗначениеПараметра = Неопределено;
		Если СтруктураПараметров.Свойство(СтрокаПараметра.Имя, ЗначениеПараметра) Тогда
			СтруктураПараметров.Удалить(СтрокаПараметра.Имя);
			
			Если Форма.Объект.ОпцияОбрабатывать__ И СтрНачинаетсяС(СтрокаПараметра.Имя, МакроПараметр) Тогда
				Продолжить;
			КонецЕсли;
			ПараметрыЗапроса_СохранитьЗначение(СтрокаПараметра.ПолучитьИдентификатор(), СтрокаПараметра.ТипЗначения.ПривестиЗначение(ЗначениеПараметра), ОбработкаОбъект);
		КонецЕсли;
	...
	Для Каждого КлючИЗначение Из СтруктураПараметров Цикл
		...
		СтрокаПараметра = Форма.ПараметрыЗапроса.Добавить();
		СтрокаПараметра.Имя = КлючИЗначение.Ключ;
		СтрокаПараметра.ТипЗначения = Новый ОписаниеТипов(МассивТипов);
		ПараметрыЗапроса_СохранитьЗначение(СтрокаПараметра.ПолучитьИдентификатор(), КлючИЗначение.Значение, ОбработкаОбъект);
