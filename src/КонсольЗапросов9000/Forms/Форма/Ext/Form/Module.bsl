...
&НаСервере
Функция ЗагрузитьВременныеТаблицы(СтруктураПараметров)
	
	Если ИспользоватьМВТ = 1 Тогда
		
		МВТ = Новый МенеджерВременныхТаблиц;
		
		ВыполнитьИнициализациюМВТ(АлгоритмИнициализацииМВТ, МВТ, СтруктураПараметров);
	...
	Иначе
		...
		Для Каждого СтрокаВременнаяТаблица Из ВременныеТаблицы Цикл
			...
			тзДанные = ОбработкаОбъект.Контейнер_ВосстановитьЗначение(СтрокаВременнаяТаблица.Контейнер);
		...
		МВТ = ЗапросЗагрузкиТаблиц.МенеджерВременныхТаблиц;
	КонецЕсли;
...
&НаСервере
Процедура СтруктураЗаписиРезультата_РаскрытьПодчиненныеУзлы()
	Перем Картинки;
	
	ТекущаяСтрока = Элементы.СтруктураЗаписиРезультата.ТекущаяСтрока;
	...
	СтрокаДерева = СтруктураЗаписиРезультата.НайтиПоИдентификатору(ТекущаяСтрока);
...
Функция ПолучитьПараметрыЗапроса()
	...
	Для Каждого СтрокаПараметра Из ПараметрыЗапроса Цикл
		...
		Значение = ПараметрыЗапроса_ПолучитьЗначение(СтрокаПараметра.ПолучитьИдентификатор(), ОбработкаОбъект);
		СтруктураПараметров.Вставить(СтрокаПараметра.Имя, Значение);
	КонецЦикла;
...
Функция ВыполнитьЗапросНаСервере(ТекстЗапроса)
	...
	СтруктураПараметров = ПолучитьПараметрыЗапроса();
	
	ВыполняемыйЗапрос = Новый Запрос;
	ВыполняемыйЗапрос.МенеджерВременныхТаблиц = ЗагрузитьВременныеТаблицы(СтруктураПараметров);
	
	Для Каждого КлючИЗначение Из СтруктураПараметров Цикл
		ВыполняемыйЗапрос.УстановитьПараметр(КлючИЗначение.Ключ, КлючИЗначение.Значение);
	КонецЦикла;
...
Процедура ПараметрыЗапросаИзСпискаЗначений(сзПараметры, ПараметрыДанныеФормыКоллекция)
	...
	Если сзПараметры <> Неопределено Тогда
		...
		Для Каждого кзПараметр Из сзПараметры Цикл
			СтрокаПараметра = ПараметрыДанныеФормыКоллекция.Добавить();
			ЗаполнитьЗначенияСвойств(СтрокаПараметра, кзПараметр.Значение);
			ЗаполнитьФорматОсобыхЗначений(СтрокаПараметра);
		КонецЦикла;
...
&НаСервере
Функция ПараметрыЗапроса_СохранитьЗначениеВызовСервера(СтрокаИд, Значение)
	ПараметрыЗапроса_СохранитьЗначение(СтрокаИд, Значение);
КонецФункции

&НаСервере
Функция ПараметрыЗапроса_ПолучитьЗначение(СтрокаИд, ОбработкаОбъект)
	...
	Если СтрокаПараметр.ТипКонтейнера = 0 ИЛИ СтрокаПараметр.ТипКонтейнера = 1 ИЛИ СтрокаПараметр.ТипКонтейнера = 2 ИЛИ СтрокаПараметр.ТипКонтейнера = 3 Тогда
		Если ОбработкаОбъект = Неопределено Тогда
			ОбработкаОбъект = РеквизитФормыВЗначение("Объект");
		КонецЕсли;
		Возврат ОбработкаОбъект.Контейнер_ВосстановитьЗначение(СтрокаПараметр.Контейнер);
	Иначе
...
&НаСервере
Функция ПараметрыЗапроса_СохранитьЗначение(СтрокаИд, Знач Значение, ОбработкаОбъект = Неопределено)
	...
	Если СтрокаПараметр.ТипКонтейнера = 0 Тогда
		СтрокаПараметр.Контейнер = ОбработкаОбъект.Контейнер_СохранитьЗначение(Значение);
...
Процедура ПараметрыЗапроса_УстановитьТип(СтрокаИд, ТипКонтейнера, ТипЗначения)
	...
	Иначе
		...
		ЗаполнитьФорматОсобыхЗначений(СтрокаПараметр);
	КонецЕсли;
	
КонецПроцедуры

&НаКлиентеНаСервереБезКонтекста
Процедура ЗаполнитьФорматОсобыхЗначений(СтрокаПараметра)
	
	ФорматЗначения = "";
	Если СтрокаПараметра.ТипКонтейнера = 0 И ТипЗнч(СтрокаПараметра.Значение) = Тип("Дата") Тогда
		ТипЧастиДаты = СтрокаПараметра.ТипЗначения.КвалификаторыДаты.ЧастиДаты;
		Если ТипЧастиДаты = ЧастиДаты.Дата Тогда
			ФорматЗначения = "Дата";
		ИначеЕсли ТипЧастиДаты = ЧастиДаты.Время Тогда
			ФорматЗначения = "Время";
		КонецЕсли;
	КонецЕсли;
	СтрокаПараметра.ФорматЗначения = ФорматЗначения;
...
Функция ПараметрыЗапроса_ПолучитьКакСтроку()
	...
	Для Каждого СтрокаПараметра Из ПараметрыЗапроса Цикл
		...
		СтрокаТаблицы.Значение = ПараметрыЗапроса_ПолучитьЗначение(СтрокаПараметра.ПолучитьИдентификатор(), ОбработкаОбъект);
	КонецЦикла;
	
	Возврат ОбработкаОбъект.ЗначениеВСтроку(тзПараметры);
...
Процедура ПоместитьРедактируемыйЗапрос()
	...
	Если РедактируемыйЗапрос >= 0 Тогда
		Если ТекстАлгоритмИзменен Тогда
			ТекстАлгоритм.УстановитьТекст(VanessaEditor().getContent());
			ТекстАлгоритмИзменен = Ложь;
		КонецЕсли;
		...
		Запрос_ПоместитьДанныеЗапроса(РедактируемыйЗапрос, стрТекстЗапроса, стрТекстАлгоритм, МетодИсполненияКода, ПараметрыЗапросаВСписокЗначений(ПараметрыЗапроса), ВременныеТаблицыВСписокЗначений(ВременныеТаблицы),
			КурсорНачалоСтроки, КурсорНачалоКолонки, КурсорКонецСтроки, КурсорКонецКолонки, КодКурсорНачалоСтроки, КодКурсорНачалоКолонки, КодКурсорКонецСтроки, КодКурсорКонецКолонки, ИспользоватьМВТ, АлгоритмИнициализацииМВТ);
	КонецЕсли;
...
Процедура ИзвлечьРедактируемыйЗапрос(ЗапросКроме = Неопределено, ВосстановитьПозийиюРедактирования = Истина)
	...
	АлгоритмИнициализацииМВТ = стДанныеЗапроса.ТекстКодМВТ;
	ИспользоватьМВТ = стДанныеЗапроса.ИспользоватьМВТ;
	
	Если Элементы.ТекстАлгоритмHTML.Видимость Тогда
		VanessaEditor().setContent(стДанныеЗапроса.ТекстКод);
	КонецЕсли;
	...
	ПриИзмененииИспользоватьМВТ(ЭтотОбъект);
...
Функция ПакетЗапросов_СтрокиВМассив(Строки)
	...
	Для Каждого Элемент Из Строки.ПолучитьЭлементы() Цикл
		стЭлемент = Новый Структура("Имя, ТекстЗапроса, ТекстКод, МетодИсполненияКода, ПараметрыЗапроса, ВременныеТаблицы, Строки, Инфо,
			...
			|КодКурсорНачалоСтроки, КодКурсорНачалоКолонки, КодКурсорКонецСтроки, КодКурсорКонецКолонки, ИспользоватьМВТ, ТекстКодМВТ");
...
Функция Запрос_ПолучитьДанныеЗапроса(идЗапрос)
	...
	Если идЗапрос = Неопределено Тогда
		Возврат Новый Структура(
			"Имя, Запрос, ТекстКод, МетодИсполненияКода, Параметры, ВКонструкторе, КурсорНачалоСтроки, КурсорНачалоКолонки, КурсорКонецСтроки, КурсорКонецКолонки, КодКурсорНачалоСтроки, КодКурсорНачалоКолонки, КодКурсорКонецСтроки, КодКурсорКонецКолонки, ИспользоватьМВТ, ТекстКодМВТ",
				"", "", "", 2, Неопределено, Ложь, 1, 1, 1, 1, 1, 1, 1, 1, 0, "");
	КонецЕсли;
	
	СтрокаЗапроса = ПакетЗапросов.НайтиПоИдентификатору(идЗапрос);
	Возврат Новый Структура(
		"Имя, Запрос, ТекстКод, МетодИсполненияКода, Параметры, ВременныеТаблицы, ВКонструкторе, КурсорНачалоСтроки, КурсорНачалоКолонки, КурсорКонецСтроки, КурсорКонецКолонки, КодКурсорНачалоСтроки, КодКурсорНачалоКолонки, КодКурсорКонецСтроки, КодКурсорКонецКолонки, ИспользоватьМВТ, ТекстКодМВТ",
		...
		СтрокаЗапроса.КодКурсорНачалоСтроки + 1, СтрокаЗапроса.КодКурсорНачалоКолонки + 1, СтрокаЗапроса.КодКурсорКонецСтроки + 1, СтрокаЗапроса.КодКурсорКонецКолонки + 1, СтрокаЗапроса.ИспользоватьМВТ, СтрокаЗапроса.ТекстКодМВТ
	);
...
Процедура Запрос_ПоместитьДанныеЗапроса(идЗапрос, стрТекстЗапроса, стрТекстКод = Неопределено, МетодИсполненияКода = Неопределено, сзПараметрыЗапроса = Неопределено, сзВременныеТаблицы = Неопределено,
		...
		КодКурсорНачалоСтроки = Неопределено, КодКурсорНачалоКолонки = Неопределено, КодКурсорКонецСтроки = Неопределено, КодКурсорКонецКолонки = Неопределено, флагИспользоватьМВТ = Неопределено, стрТекстКодМВТ = Неопределено)
	...
	Если флагИспользоватьМВТ <> Неопределено Тогда
		СтрокаЗапроса.ИспользоватьМВТ = флагИспользоватьМВТ;
	КонецЕсли;
	
	Если стрТекстКодМВТ <> Неопределено Тогда
		СтрокаЗапроса.ТекстКодМВТ = стрТекстКодМВТ;
	КонецЕсли;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИнициализацияРедактораКода(ОбъектОбработки);
...
Процедура ИнициализацияРедактораКода(ОбработкаОбъект)
	
	ИмяВременногоФайла = ПолучитьИмяВременногоФайла();
	УдалитьФайлы(ИмяВременногоФайла);
	СоздатьКаталог(ИмяВременногоФайла);
	
	МакетДвочиныеДанные = ОбработкаОбъект.ПолучитьМакет("VanessaEditor");
	
	ЧтениеZipФайла = Новый ЧтениеZipФайла(МакетДвочиныеДанные.ОткрытьПотокДляЧтения());
	Для Каждого ЭлементZipФайла Из ЧтениеZipФайла.Элементы Цикл
		ЧтениеZipФайла.Извлечь(ЭлементZipФайла, ИмяВременногоФайла, РежимВосстановленияПутейФайловZIP.Восстанавливать);
		ДвоичныеДанные = Новый ДвоичныеДанные(ИмяВременногоФайла + ПолучитьРазделительПути() + ЭлементZipФайла.ПолноеИмя);
		VanessaEditorURL = ПолучитьНавигационнуюСсылкуИнформационнойБазы() + "/" + ПоместитьВоВременноеХранилище(ДвоичныеДанные, УникальныйИдентификатор)
			+ "&localeCode=" + Лев(ТекущийЯзыкСистемы(), 2);
	EndDo;
	УдалитьФайлы(ИмяВременногоФайла);
	
	Элементы.ТекстАлгоритм.Видимость = Ложь;
	Элементы.ТекстАлгоритмHTML.Видимость = Истина;
	...
	Если РезультатЗакрытия <> Неопределено Тогда
		...
		Если ДополнительныеПараметры.Поле = "Контейнер" Тогда
			Если ДополнительныеПараметры.Таблица = "ПараметрыЗапроса" Тогда
				ПараметрыЗапроса_СохранитьЗначениеВызовСервера(ДополнительныеПараметры.Строка, РезультатЗакрытия.Значение);
		...
		ИначеЕсли ДополнительныеПараметры.Поле = "КонтейнерКакТип" Тогда
			ПараметрыЗапроса_СохранитьЗначениеВызовСервера(ДополнительныеПараметры.Строка, РезультатЗакрытия.ОписаниеКонтейнера);
...
Процедура ПараметрыЗапросаЗначениеОкончаниеВводаТекста(Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных, СтандартнаяОбработка)
	...
	Если ТипЗнч(ТекущиеДанные.Контейнер) = Тип("Структура") И ТекущиеДанные.Контейнер.Тип = "УникальныйИдентификатор" Тогда
		...
		КонецПопытки;
		ПараметрыЗапроса_СохранитьЗначениеВызовСервера(Элементы.ПараметрыЗапроса.ТекущаяСтрока, Значение);
		СтандартнаяОбработка = Ложь;
	КонецЕсли;
КонецПроцедуры
...
Процедура СтруктураЗаписиРезультатаПередРазворачиванием(Элемент, Строка, Отказ)
	...
	Если НЕ СтрокаДерева.ПодчиненныеУзлыРаскрыты Тогда
		ПодключитьОбработчикОжидания("СтруктураЗаписиРезультатаПередРазворачиваниемВызовСервера", 0.1, Истина);
	Конецесли;
	
КонецПроцедуры

&НаКлиенте
Процедура СтруктураЗаписиРезультатаПередРазворачиваниемВызовСервера()
	СтруктураЗаписиРезультата_РаскрытьПодчиненныеУзлы();
КонецПроцедуры
...
&НаКлиенте
Процедура ТекстАлгоритмHTMLДокументСформирован(Элемент)
	
	Editor = DefaultView().createVanessaEditor(ТекстАлгоритм.ПолучитьТекст(), "bsl");
	Editor.showMinimap(Ложь);
	
КонецПроцедуры

&НаКлиенте
Процедура ТекстАлгоритмHTMLПриНажатии(Элемент, ДанныеСобытия, СтандартнаяОбработка)
	
	Element = ДанныеСобытия.Element;
	Если Element.id = "VanessaEditorEventForwarder" Тогда
		ИзмененияАлгоритма = Ложь;
		Пока Истина Цикл
			ОписаниеСобытия = DefaultView().popVanessaMessage();
			Если ОписаниеСобытия = Неопределено Тогда
				Прервать;
			КонецЕсли;
			Если ОписаниеСобытия.type = "CONTENT_DID_CHANGE" Тогда
				Модифицированность = Истина;
				ТекстАлгоритмИзменен = Истина;
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
...
Функция ЗаполнитьИзЧтениеXML(ЧтениеXML)
	...
	ОбновитьПараметрыЗапросаПоСтруктуре(стрТекстЗапроса, стПараметрыЗапроса, ОбработкаОбъект);
	
	//Чтение временных таблиц.
	маТаблицы = Неопределено;
	Если СтруктураПараметров.Свойство("ВременныеТаблицы", маТаблицы) Тогда
		ЗаполнитьВременныеТаблицы(маТаблицы, ОбработкаОбъект);
	КонецЕсли;
...
Процедура ВыполнитьЗапрос(фИспользоватьВыделение)
	...
	ТекущийЗапрос = ПакетЗапросов.НайтиПоИдентификатору(Элементы.ПакетЗапросов.ТекущаяСтрока);
	...
	Иначе
		...
		ТекущийЗапрос.Инфо = Строка(КоличествоВозвращенныхСтрокРезультата) + " / " + ВремяВыполнения;
		
	КонецЕсли;
	
	ИмяЗапросаРезультата = ТекущийЗапрос.Имя;
	ОбновитьСостояниеЭлементовФормыАлгоритма();
...
Функция ПараметрыЗаполнитьИзЗапросаНаСервере()
	...
	Для Каждого Параметр Из НайденныеПараметры Цикл
		...
		Если маСтрокиПараметра.Количество() > 0 Тогда
			...
			Если СтрокаПараметра.ТипКонтейнера = 3 Тогда
				ТекущаяТЗ = ПараметрыЗапроса_ПолучитьЗначение(СтрокаПараметра.ПолучитьИдентификатор(), ОбработкаОбъект);
				НоваяТЗ = ПолучитьОписаниеПараметраТаблицаЗначений(Запрос.Текст, Параметр.Имя);
				Для Каждого НоваяКолонка Из НоваяТЗ.Колонки Цикл
					Если ТекущаяТЗ.Колонки.Найти(НоваяКолонка.Имя) = Неопределено Тогда
						ТекущаяТЗ.Колонки.Добавить(НоваяКолонка.Имя, НоваяКолонка.ТипЗначения);
					КонецЕсли;
				КонецЦикла;
				ПараметрыЗапроса_СохранитьЗначение(СтрокаПараметра.ПолучитьИдентификатор(), ТекущаяТЗ, ОбработкаОбъект);
			КонецЕсли;
		Иначе
			...
			Если Параметр.ТипЗначения.СодержитТип(Тип("ТаблицаЗначений")) Тогда
				...
				ОписаниеТЗ = ПолучитьОписаниеПараметраТаблицаЗначений(Запрос.Текст, Параметр.Имя);
				ПараметрыЗапроса_СохранитьЗначение(СтрокаПараметра.ПолучитьИдентификатор(), ОписаниеТЗ, ОбработкаОбъект);
			Иначе
				ПараметрыЗапроса_СохранитьЗначение(СтрокаПараметра.ПолучитьИдентификатор(), Параметр.ТипЗначения.ПривестиЗначение(Неопределено), ОбработкаОбъект);
			КонецЕсли;
		КонецЕсли;
...
Функция ПолучитьОписаниеПараметраТаблицаЗначений(ТекстЗапроса, ИмяПараметра)
	
	СхемаЗапроса = Новый СхемаЗапроса;
	СхемаЗапроса.УстановитьТекстЗапроса(ТекстЗапроса);
	
	ОписаниеТЗ = Новый ТаблицаЗначений;
	
	Для Каждого ОписаниеПакетЗапросов Из СхемаЗапроса.ПакетЗапросов Цикл
		Для Каждого ОписаниеОператор Из ОписаниеПакетЗапросов.Операторы Цикл
			Для Каждого ОписаниеИсточник Из ОписаниеОператор.Источники Цикл
				Если ТипЗнч(ОписаниеИсточник.Источник) = Тип("ОписаниеВременнойТаблицыСхемыЗапроса")
				   И ОписаниеИсточник.Источник.ИмяТаблицы = "&" + ИмяПараметра Тогда
				Для Каждого ОписаниеПоля Из ОписаниеИсточник.Источник.ДоступныеПоля Цикл
					Если ОписаниеТЗ.Колонки.Найти(ОписаниеПоля.Имя) = Неопределено Тогда
						ОписаниеТЗ.Колонки.Добавить(ОписаниеПоля.Имя, ОписаниеПоля.ТипЗначения);
					КонецЕсли;
				КонецЦикла;
				КонецЕсли;
			КонецЦикла;
...
Процедура Команда_ДобавитьПереносСтрокВТекст(Команда)
	...
	ИначеЕсли Элементы.ГруппаЗапросСтраницы.ТекущаяСтраница = Элементы.СтраницаАлгоритмЗаполнения Тогда
		ДобавитьПереносСтрокВТекст(Элементы.АлгоритмЗаполнения);
	ИначеЕсли Элементы.ГруппаЗапросСтраницы.ТекущаяСтраница = Элементы.СтраницаВременныеТаблицы
	        и ИспользоватьМВТ = 1 Тогда
		ДобавитьПереносСтрокВТекст(Элементы.АлгоритмИнициализацииМВТ);
	КонецЕсли;
КонецПроцедуры
...
Процедура Команда_УбратьПереносСтрокИзТекста(Команда)
	...
	ИначеЕсли Элементы.ГруппаЗапросСтраницы.ТекущаяСтраница = Элементы.СтраницаАлгоритмЗаполнения Тогда
		УбратьПереносСтрокИзТекста(Элементы.АлгоритмЗаполнения);
	ИначеЕсли Элементы.ГруппаЗапросСтраницы.ТекущаяСтраница = Элементы.СтраницаВременныеТаблицы
	        и ИспользоватьМВТ = 1 Тогда
		УбратьПереносСтрокИзТекста(Элементы.АлгоритмИнициализацииМВТ);
	КонецЕсли;
КонецПроцедуры
...
Процедура Команда_ДобавитьКомментированиеСтрокВТекст(Команда)
	...
	ИначеЕсли Элементы.ГруппаЗапросСтраницы.ТекущаяСтраница = Элементы.СтраницаАлгоритмЗаполнения Тогда
		ДобавитьКомментированиеСтрокВТекст(Элементы.АлгоритмЗаполнения);
	ИначеЕсли Элементы.ГруппаЗапросСтраницы.ТекущаяСтраница = Элементы.СтраницаВременныеТаблицы
	        и ИспользоватьМВТ = 1 Тогда
		ДобавитьКомментированиеСтрокВТекст(Элементы.АлгоритмИнициализацииМВТ);
	КонецЕсли;
КонецПроцедуры
...
Процедура Команда_УбратьКомментированиеСтрокИзТекста(Команда)
	...
	ИначеЕсли Элементы.ГруппаЗапросСтраницы.ТекущаяСтраница = Элементы.СтраницаАлгоритмЗаполнения Тогда
		УбратьКомментированиеСтрокИзТекста(Элементы.АлгоритмЗаполнения);
	ИначеЕсли Элементы.ГруппаЗапросСтраницы.ТекущаяСтраница = Элементы.СтраницаВременныеТаблицы
	        и ИспользоватьМВТ = 1 Тогда
		УбратьКомментированиеСтрокИзТекста(Элементы.АлгоритмИнициализацииМВТ);
	КонецЕсли;
КонецПроцедуры
...
Процедура Команда_ПолучитьКодДляПерехватаВОтладчике(Команда)
	...
	Если Объект.РежимЗапускаВнешняяОбработка Тогда
		...
		стрКодСКД = СтрШаблон("ВнешниеОбработки.Создать(""%1"", Ложь).СохранитьЗапросСКД(%2, СхемаКомпоновкиДанных, КомпоновщикНастроек)", стрИмяФайлаОбработкиНаСервере, Формат(Объект.СеансИД, "ЧГ=0"));
		
	Иначе
		
		стрКод = СтрШаблон("Обработки.%1.Создать().СохранитьЗапрос(%2, Запрос)", Объект.ИмяОбработки, Формат(Объект.СеансИД, "ЧГ=0"));
		стрКодСКД = СтрШаблон("Обработки.%1.Создать().СохранитьЗапросСКД(%2, СхемаКомпоновкиДанных, КомпоновщикНастроек)", Объект.ИмяОбработки, Формат(Объект.СеансИД, "ЧГ=0"));
...
&НаСервереБезКонтекста
Функция ВыполнитьИнициализациюМВТ(ЭтотКод, МенеджерВременныхТаблиц, Параметры)
	Выполнить(ЭтотКод);
КонецФункции
...
&НаКлиенте
Процедура ИспользоватьМВТПриИзменении(Элемент)
	ПриИзмененииИспользоватьМВТ(ЭтотОбъект);
КонецПроцедуры

&НаКлиентеНаСервереБезКонтекста
Процедура ПриИзмененииИспользоватьМВТ(Форма)
	
	Если Форма.ИспользоватьМВТ = 1 Тогда
		Форма.Элементы.ГруппаНастройкиМВТ.ТекущаяСтраница = Форма.Элементы.ГруппаАлгоритмИницализации;
	Иначе
		Форма.Элементы.ГруппаНастройкиМВТ.ТекущаяСтраница = Форма.Элементы.ГруппаМенеджерВременныхТаблиц;
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Процедура Команда_ПереключитьВидимостьСпискаЗапросов(Команда)
	Элементы.ВидимостьСпискаЗапросов.Пометка = НЕ Элементы.ВидимостьСпискаЗапросов.Пометка;
	Элементы.ГруппаСписокЗапросов.Видимость = Элементы.ВидимостьНастроекЗапроса.Пометка И Элементы.ВидимостьСпискаЗапросов.Пометка;
КонецПроцедуры

&НаКлиенте
Процедура Команда_ПереключитьВидимостьНастроекЗапроса(Команда)
	Элементы.ВидимостьНастроекЗапроса.Пометка = НЕ Элементы.ВидимостьНастроекЗапроса.Пометка;
	Элементы.ГруппаСписокЗапросов.Видимость = Элементы.ВидимостьНастроекЗапроса.Пометка И Элементы.ВидимостьСпискаЗапросов.Пометка;
	Элементы.ВидимостьСпискаЗапросов.Доступность = Элементы.ВидимостьНастроекЗапроса.Пометка;
	Элементы.ГруппаНижняяПанель.Видимость = Элементы.ВидимостьНастроекЗапроса.Пометка;
КонецПроцедуры

&НаКлиенте
Процедура Команда_ВыполнитьАлгоритмЗаполнения(Команда)
	ВыполнитьАлгоритмЗаполненияНаСервере();
КонецПроцедуры

&НаСервере
Процедура ВыполнитьАлгоритмЗаполненияНаСервере()
	
	СтруктураПараметров = ПолучитьПараметрыЗапроса();
	ТекстЗапросаСтрокой = ТекстЗапроса.ПолучитьТекст();
	МенеджерВременныхТаблиц = Неопределено;
	БезопасноВыполнитьАлгоритмЗаполненияНаСервере(ТекстКодЗаполнения, ТекстЗапросаСтрокой, СтруктураПараметров, МенеджерВременныхТаблиц);
	
	ОбработкаОбъект = РеквизитФормыВЗначение("Объект");
	
	ОбновитьПараметрыЗапросаПоСтруктуре(ТекстЗапросаСтрокой, СтруктураПараметров, ОбработкаОбъект);
	ДанныеВременныхТаблиц = ОбработкаОбъект.ДанныеВременныхТаблиц(МенеджерВременныхТаблиц);
	Если ДанныеВременныхТаблиц <> Неопределено Тогда
		ЗаполнитьВременныеТаблицы(ДанныеВременныхТаблиц, ОбработкаОбъект);
		Если ИспользоватьМВТ = 1 Тогда
			ИспользоватьМВТ = 0;
			ПриИзмененииИспользоватьМВТ(ЭтотОбъект);
		КонецЕсли;
...
&НаСервереБезКонтекста
Процедура БезопасноВыполнитьАлгоритмЗаполненияНаСервере(ЭтотКод, Текст, Параметры, МенеджерВременныхТаблиц)
	Выполнить(ЭтотКод);
КонецПроцедуры

&НаСервере
Процедура ОбновитьПараметрыЗапросаПоСтруктуре(стрТекстЗапроса, стПараметрыЗапроса, ОбработкаОбъект)
	...
	Для Каждого кзПараметр Из стПараметрыЗапроса Цикл
		...
		ПараметрыЗапроса_СохранитьЗначение(СтрокаПараметра.ПолучитьИдентификатор(), кзПараметр.Значение, ОбработкаОбъект);
		ЗаполнитьФорматОсобыхЗначений(СтрокаПараметра);
...
Процедура ЗаполнитьВременныеТаблицы(маТаблицы, ОбработкаОбъект)
	...
	Для Каждого стТаблица Из маТаблицы Цикл
		...
		Для Каждого кз Из стНовыеТипыКолонок Цикл
			ОбработкаОбъект.ИзменитьТипКолонкиТаблицыЗначений(тзТаблица, кз.Ключ, кз.Значение);
		КонецЦикла;
		
		СтрокаТаблицы = ВременныеТаблицы.Добавить();
		СтрокаТаблицы.Имя = стТаблица.Имя;
		СтрокаТаблицы.Контейнер = ОбработкаОбъект.Контейнер_СохранитьЗначение(тзТаблица);
		СтрокаТаблицы.Значение = СтрокаТаблицы.Контейнер.Представление;
...
&НаКлиенте
Функция DefaultView()
	Возврат Элементы.ТекстАлгоритмHTML.Document.defaultView;
КонецФункции

&НаКлиенте
Function VanessaEditor()
	
	Возврат DefaultView().VanessaEditor;
