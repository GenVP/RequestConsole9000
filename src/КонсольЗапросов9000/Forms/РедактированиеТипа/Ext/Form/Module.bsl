...
Процедура ДобавитьТипыСсылок(Типы, ТипЗначенияТекущий, Менеджер, стрПрефиксИмени, Картинки)
	...
	Для Каждого ОбъектМД Из Метаданные[Менеджер] Цикл
		Ссылка = Новый (стрПрефиксИмени + "." + ОбъектМД.Имя);
		стрИмяТипа = ОбъектМД.Имя;
		стрТип = стрПрефиксИмени + "." + стрИмяТипа;
		ДобавитьТип(ВеткаТипов, ТипЗначенияТекущий, стрТип, КартинкаСсылок, стрИмяТипа);
	КонецЦикла;
	
	ВеткаТипов.Представление = стрПрефиксИмени + " (" + Формат(ВеткаТипов.Строки.Количество(), "ЧГ=0") + ")";
