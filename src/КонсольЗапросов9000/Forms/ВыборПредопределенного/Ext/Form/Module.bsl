...
Процедура ТипОбъектаПриИзмененииНаСервере()
	...
	ИначеЕсли ТипОбъекта = "ВидСчета" Тогда
		сзСписокОбъектов.Добавить(ВидСчета.АктивноПассивный, "Активный/Пассивный");
...
Процедура ИмяОбъектаПриИзмененииНаСервере()
	...
	Если ЗначениеЗаполнено(ИмяОбъекта) Тогда
		...
		Если ТипОбъекта = "Справочники"
		 ИЛИ ТипОбъекта = "ПланыВидовХарактеристик"
		 ИЛИ ТипОбъекта = "ПланыСчетов"
		 ИЛИ ТипОбъекта = "ПланыВидовРасчета"
		 ИЛИ ТипОбъекта = "ПланыОбмена" Тогда
			зЗапрос = Новый Запрос(СтрШаблон("ВЫБРАТЬ Ссылка ИЗ %1.%2 ГДЕ Предопределенный", ТипДляЗапроса(ТипОбъекта), ИмяОбъекта));
...
Процедура КомандаВставитьЗНАЧЕНИЕ(Команда)
	СохранитьДанныеФормы();
	Если ТипОбъектаПеречисление(ТипОбъекта) Тогда
		Результат = "ЗНАЧЕНИЕ(" + ПолучитьПолноеИмяПредопределенногоЗначения(ИмяОбъекта) + ")";
	Иначе
		Результат = "ЗНАЧЕНИЕ(" + ПолучитьПолноеИмяПредопределенногоЗначения(Элемент) + ")";
	КонецЕсли;
	Закрыть(Новый Структура("ДанныеФормы, Результат", ДанныеФормы, Результат));
КонецПроцедуры
